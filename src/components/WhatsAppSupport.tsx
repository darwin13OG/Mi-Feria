...
export const WhatsAppSupport: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-40 no-print">
      {/* Sliding and Scaling Quick Contact Panel */}
      <div
        className={`mb-3 w-80 rounded-3xl border p-4 shadow-2xl backdrop-blur-xl bg-[#0c0c10]/95 border-emerald-500/30 text-white transition-all duration-300 origin-bottom-right ${
          isOpen
            ? 'opacity-100 scale-100 translate-y-0 pointer-events-auto shadow-emerald-950/40'
          ...
          {quickActions.map((qa, i) => {
            ...
            return (
              <a
                ...
                onClick={() => setIsOpen(false)}
                className="group flex items-start gap-2.5 p-2.5 rounded-2xl bg-white/5 hover:bg-emerald-500/10 border border-white/5 hover:border-emerald-500/30 transition-all text-left block"
              >
      ...
      <div className="flex items-center justify-end gap-2">
        ...
        <button
          ...
          className={`relative flex items-center justify-center w-14 h-14 rounded-full shadow-2xl transition-all duration-300 transform active:scale-90 focus:outline-none ${
            isOpen
              ? 'bg-zinc-800 text-white rotate-90 border border-white/20'
              : 'bg-gradient-to-tr from-emerald-600 via-emerald-500 to-teal-400 text-white hover:scale-110 shadow-emerald-500/30 ring-4 ring-emerald-500/20'
          }`}
        >
          {isOpen ? (
            ...
            <>
              ...
              <MessageCircle className="w-7 h-7 text-white relative z-10 transition-transform duration-200 group-hover:scale-110" />
            </>
          )}
        </button>
      </div>
